...
export class ViajesreservadosPage implements OnInit {
  ...
  getViaje(idViaje: string) {
    return this.arregloViajes.find((viaje: any) => viaje.id_viaje === idViaje);
  }
  
  getConductor(idUsuario: string) {
    return this.arregloUsuario.find((usuario: any) => usuario.id === idUsuario);
  }
  
  getVehiculo(idUsuario: string) {
    return this.arregloVehiculo.find((vehiculo: any) => vehiculo.id_usuario === idUsuario);
  }
