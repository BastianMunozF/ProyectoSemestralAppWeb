                        ...
                        this.database.fetchConductor().subscribe(usuarios => {
                          if (usuarios.length > 0) {
                            ...
                            this.database.buscarVehiculoUsuario(usuarios[0].id).then(vehiculo => {
                              ...
                              this.presentarAlerta("Error al cargar datos", "Error en función buscar vehículo usuario.");
                            })
                          } else {
                            this.presentarAlerta("Error al cargar datos", "Error en función fetch conductor.");
                          }
                        })
                      } else {
                        this.presentarAlerta("Error al cargar datos", "Error en función buscar datos conductor.");
                      }
                    }).catch(error => {
                      console.log('Error en Buscar Datos Usuario: ', error);
                      this.presentarAlerta("Error al cargar datos", "Error en función buscar datos usuario.");
                    })
                  } else {
                    this.presentarAlerta("Error aquí", "Error en función fetch viaje reservado.");
                  }
                })
              } else {
                this.presentarAlerta("Error aquí", "Error en función buscar viaje reservado.");
              }
            }).catch(error => {
