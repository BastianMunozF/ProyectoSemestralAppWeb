      ...
      if(res.length > 0){
        this.database.fetchDetalleUser().subscribe(detail => {
          if(detail.length > 0){
            ...
            this.database.buscarViajeReservado(parseInt(detail[2].id_viaje), estado).then(viaje => {
              if(viaje.length > 0){
                this.database.fetchViajeReservado().subscribe(viajes => {
                  if(viajes.length > 0){
                    ...
                    this.database.buscarDatosConductor(parseInt(viajes[0].id_usuario)).then(usuario => {
                      if(usuario.length > 0){
                        this.database.fetchConductor().subscribe(usuarios => {
                          if(usuarios.length > 0){
                            ...
                            this.database.buscarVehiculoUsuario(parseInt(usuarios[0].id)).then(vehiculo => {
